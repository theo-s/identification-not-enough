# This runs a very simular simulation to the original 1 - dimensional
# experiment 3 from the paper. In order to examine a different version of this,
# we swap the order of the step function if W > .3
...
nonlinear_ps <- function(x) {
  ...
  # In order to examine a different version of this,
  # we swap the order of the step function if W > .3
  n_bins <- 100*length(x)
  bin_width <- 1/n_bins
...
nonlinear_ps_swap <- function(x) {
  ...
  bin_width <- 1/n_bins
  probs <- ifelse(x > .3,
                  ifelse(x %% bin_width < (bin_width/2),
                         .9,
                         .1),
                  ifelse(x %% bin_width < (bin_width/2),
                         .1,
...
for (n in c(1e3,1e4,1e5)) {
  experiment_1 <- run_sim(p_score = nonlinear_ps,
                          mu_1 = nonlinear,
                          n=n,
                          seed=seed)
...
filename <- paste0("code/results_exp5/res_",seed,".RDS")
